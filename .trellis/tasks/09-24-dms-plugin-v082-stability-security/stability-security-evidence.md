...
- Offscreen: not run. `qmllint`, `qmlformat`, and `qmltestrunner` were not found
  on `PATH`.
- Live: not run. No `dms` or `quickshell` process was present. The installed
  CLI reports DMS 1.6.2; the roadmap target is 1.6.1.

Consequently, DMS 1.6.1 compatibility, live idle activity, task refresh
latency, topology/settings/manual refresh, reload/destruction cleanup,
multi-widget/multi-screen behavior, and live empty/malformed/inaccessible
shell recovery remain **UNVERIFIED**. Large-data fixtures establish the input
sizes and parser/detail branches, but do not measure peak memory, latency, or
simultaneous process/reader counts.
