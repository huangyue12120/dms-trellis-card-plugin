...
- A user-run DMS 1.6.2 session first exposed `PluginGlobalVar is not a type` because `TrellisLauncher.qml` omitted `import qs.Widgets`; after that import was copied, DMS exposed `Cannot assign to non-existent default property` because the Launcher root was `QtObject`. The import and root are now corrected (`Item`), with source-contract assertions for both. `node tests/test_trellis_contract.mjs`, `node --check tests/test_trellis_contract.mjs`, and `git diff --check` pass. After the user copied the `Item`-root file, the 22:39 DMS startup log reports the Trellis bar widget and daemon loaded without a new component error. Actual DMS Launcher trigger/search, selection, State, popout, and reload behavior remains unverified because `!trellis` was entered in zsh, not the Launcher UI.
