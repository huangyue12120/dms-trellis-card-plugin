...
- A user-run DMS 1.6.2 session exposed `PluginGlobalVar is not a type` at the Launcher instance because `TrellisLauncher.qml` omitted `import qs.Widgets`. The import and a source-contract regression assertion were added; `node tests/test_trellis_contract.mjs`, `node --check tests/test_trellis_contract.mjs`, and `git diff --check` pass. The corrected Launcher still needs host verification for load, search, selection, State, popout, and reload behavior.
