...
- A user-run DMS 1.6.2 session first exposed `PluginGlobalVar is not a type` because `TrellisLauncher.qml` omitted `import qs.Widgets`; after that import was copied, DMS exposed `Cannot assign to non-existent default property` because the Launcher root was `QtObject`. The import and root are now corrected (`Item`), with source-contract assertions for both. `node tests/test_trellis_contract.mjs`, `node --check tests/test_trellis_contract.mjs`, and `git diff --check` pass. The latest root fix still needs host verification for load, search, selection, State, popout, and reload behavior.
